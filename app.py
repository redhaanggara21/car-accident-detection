    ...
    if request.method == 'POST':
        ...
        if connection:
            try:
                ...
                #     query = (f"INSERT INTO `car_damage_detection`.`user_info` (`name`, `password`, `email`, `vehicle_id`, `contact_number`, `address`, `car_brand`, `model`)  VALUES (%s, %s, %s, %s, %s, %s, %s, %s)")
                #     cursor.execute(query, (name, hashed_password, email, vehicle_id, contact_number, address, car_brand, model))
                #     connection.commit()
                # flash("Signup successful!", "success")
                c = connection.cursor()
                query = (f"INSERT INTO `car_damage_detection`.`user_info` (`name`, `password`, `email`, `vehicle_id`, `contact_number`, `address`, `car_brand`, `model`)  VALUES (%s, %s, %s, %s, %s, %s, %s, %s)")
                c.execute(query, (name, hashed_password, email, vehicle_id, contact_number, address, car_brand, model))
                connection.commit()
                flash("Signup successful!", "success")
...
# Load YOLO model
model_path = "../accident-damage-detection/models/best.pt"
...
def get_part_prices(email, class_counts):
    ...
    if connection:
        try:
            ...
            car_brand = user_data[0]
            car_model = user_data[1]
